...
众所周知， Java8 在一定程度上支持了函数式编程，但标准库提供的函数式 API 不是很完备和友好，为了更好的进行函数式编程，我们就不得不借助于第三方库，而 VAVR 就是这方面的佼佼者，它可以有效减少代码量并提高代码质量。

用官方的语言来说， VAVR 就是一个用 Java 编写的函数式扩展库。

看到这儿，很多人就说我标题党了，**一个 Java 库还来颠覆 Java**  ？

这我就得喊冤了，可不不是我玩**震惊体**，打开 VAVR 的[官网](https://www.vavr.io/) ，它首页就以加粗字体写着 「**vavr** - turns java**™** upside down」
...
这篇文章会基于读者当前状况产生不同的营养价值

- 如果你还处于 Java7 及其以前，那么本文营养价值不菲，但有点吸收难度（需要了解一些 Java8 的语法和 API）
- 如果你刚来到  java8，那么本文营养价值极高，就算撑不下，看一眼也有收获
- 如果你来自 Scala 或其他 FP 语言，正在忍受 Java，那么本文营养价值中等
- 其余人士，难以评估，默认无穷高



由于是篇框架介绍文（地推 ing），烹饪时有些原则，请悉知

- 为了避免写成官方文档的翻译文，我不会介绍所有的特性和 API，仅做抛砖引玉
- 这不是 FP  原理的文章，会尽量避免 Side-Effects、Monad 等概念（实际上我也不懂）
- 很多代码段，以单元测试的形式给出
