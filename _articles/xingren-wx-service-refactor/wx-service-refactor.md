...
现在，组件层次和职责是划分清楚了，但是实际的设计中还存在一个问题：

**从 Controller 到 WX-Api 的流转中，共用了一部分对象模型**

这就是下面要提到的 “不恰当的模型共用”

### 不恰当的模型共用

一图胜千言，先来看看所谓的**模型对象共用**是什么情况



![](img/same-model.svg)

上图中， 各层次的组件都共用了一套模型对象，为什么会出现这样的情况呢？

其实是因为 Controller 中某个方法的参数和返回值与 WX-Api 组件中的参数和返回值字段一样，所以就共用了。

这样即减少了类定义，又加快了开发效率，有何不妥呢？

不妥之处在于违反了单一职责

> 单一职责：一个类应该只有发生变化的原因

- WX-Api 的变更原因主要和微信官方 API 的迭代相关
- Controller / Service 的变更原因主要和公司业务相关

多个组件都可能因为自己的变更原因去修改**模型对象**，最终必然导致模型对象成长为四不像，各个组件的变更也会捉襟见肘。

而为了考虑重构成本（是懒吗~）

- WX-Api 组件使用独立的模型
- Controller / Service 使用另外一套模型

- WX-Api 与 Service 组件之间的交互需要将模型转换为自有模型

这样定义以后，交互图大概就变成了下面这样

![image-20200510002006737](img/dependent-model.svg)

当然，理想状态下， Controller 和 Service 之间的模型也应该独立开来，可以思考一下为什么？

解决了共用模型的问题，却还有另外一个问题：**组件内部的类，允许互相依赖吗？**

在严格分层模式下，是不允许的，但实际情况下总会有打破严格分层约束的情况。

而同层组件的类相互依赖，又会造成循环依赖，恰恰这次要重构的服务中就有这样的情况。
...
以 `ThirdService`  和  `TokenService` 两个类为例
